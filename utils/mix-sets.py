...
train_input = open("train-input-3", 'w')
train_output = open("train-output-3", 'w')
test_input = open("test-input-3", 'w')
test_output = open("test-output-3", 'w')

for kind in kinds:
    data_files.append(open(kind + '.bin'))
...
while True:
    ...
    for j in range(kinds_count):
        ...
        if line:
            if i % 10 < 8: 
                print(line, file=train_input)
                print(str(j), file=train_output)
            else:
                print(line, file=test_input)
